...
int main(int argc, char** argv) {
  if (argc != 6) {
    std::cerr << "usage: face_tracking_bringup CAMERA_EXECUTABLE DETECTOR_EXECUTABLE CONTROLLER_EXECUTABLE SERVO_EXECUTABLE HMI_PYTHON\n";
    return 2;
  }
  ...
  try {
    ...
    children.push_back(spawn({argv[4]}));
    children.push_back(spawn({argv[5], "-s", "-m", "face_tracking_hmi"}));
    int child_status = 0;
    while (!interrupted) {
