        ...
        with zenoh.open(config) as session:
            def on_frame(sample: object) -> None:
                try:
                    ...
                    self._store.update(bytes(getattr(sample, "payload")), decode_frame_metadata(bytes(attachment)))
                except (ProtocolError, TypeError, ValueError):
                    self._store.reject_invalid_frame()

            def on_status(sample: object) -> None:
                try:
                    self._store.update_driver_status(decode_camera_status(bytes(getattr(sample, "payload"))))
                except (ProtocolError, TypeError, ValueError):
                    self._store.reject_invalid_frame()

            def on_detection(sample: object) -> None:
                try:
                    self._store.update_detection(decode_detection(bytes(getattr(sample, "payload"))))
                except (ProtocolError, TypeError, ValueError):
                    self._store.reject_invalid_frame()
...
def create_transport(settings: Settings, store: LatestFrameStore) -> TransportAdapter:
    if settings.adapter == "zenoh":
        return ZenohTransport(settings, store)
    raise RuntimeError(f"unsupported middleware adapter: {settings.adapter}")
